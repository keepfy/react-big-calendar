    ...
    function EventContainerWrapper() {
      ...
      _this.handleDropFromOutside = function(point, boundaryBox) {
        ...
        if (
          ...
          _this.context.draggable.dragAndDropAction.isLockedDropFromOutside
        )
          return
      ...
      _this._selectable = function() {
        ...
        selector.on('selecting', function(box) {
          ...
          var dragAndDropAction = _this.context.draggable.dragAndDropAction
          var action = dragAndDropAction.action,
            direction = dragAndDropAction.direction
          if (action === 'move') _this.handleMove(box, bounds)
          if (
            action === 'resize' &&
        ...
        selector.on('select', function(point) {
          ...
          if (!_this.state.event || !pointInColumn(bounds, point)) {
            ...
            if (_this.context.draggable.dragAndDropAction.interacting)
              _this.context.draggable.onEnd(null, true)
            return
          }
...
EventContainerWrapper.contextTypes = {
  draggable: _propTypes.default.shape({
    ...
    lockDropFromOutside: _propTypes.default.func,
